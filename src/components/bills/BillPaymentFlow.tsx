...
function BillPaymentContent({
  ...
  const { data: items, isLoading: itemsLoading } = useQuery({
    queryKey: ["biller-items", biller?.biller_code],
    queryFn: () => getBillerItems(biller!.biller_code),
    enabled: !!biller,
  });
  ...
  const { mutate: validate, isPending: validating } = useMutation({
    mutationFn: ({ customer }: { customer: string }) => validateCustomer(item!.item_code, customer),
    onSuccess: (data) => {
      setValidation(data);
      setStep("confirm");
    },
  ...
  const { mutate: pay, isPending: paying } = useMutation({
    mutationFn: (data: PayForm) =>
      payBill(biller!.biller_code, item!.item_code, {
  ...
  if (step === "success") {
    return (
      <div className="flex flex-col items-center text-center py-6 gap-4 animate-fade-in">
        ...
        <p className="text-slate-500 text-sm">Your {category.name} bill has been paid.</p>
        <Button fullWidth onClick={onSuccess} id="bill-done">
          Done
        </Button>
      </div>
    );
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        {["Biller", "Package", "Details", "Confirm"].map((label, i) => (
          <div key={label} className="flex-1 flex flex-col items-center gap-1">
            ...
            />
            <span
              className={`text-[10px] font-semibold ${i <= stepIdx ? "text-indigo-600" : "text-slate-300"}`}
            >
              {label}
            </span>
          </div>
      ...
      {step === "item" && (
        <div className="space-y-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => setStep("biller")}>
            ← Back
          </Button>
        </div>
      )}
