  ...
  return (
    <Tabs.Root value={tab} onValueChange={setTab}>
      ...
      <Tabs.Content value="card">
        <form
          onSubmit={handleSubmit((d) => mutate({ cardId: d.cardId, amount: d.amount }))}
          className="space-y-4"
        >
      ...
      <Tabs.Content value="transfer">
        <div className="space-y-4">
          <div className="rounded-2xl gradient-brand p-5 text-white">
            ...
            <p className="font-bold mb-4">{wallet?.bankName ?? "--"}</p>
          ...
          <div className="flex items-start gap-2 rounded-xl bg-green-50 border border-green-100 p-3">
            ...
            <p className="text-xs text-green-700">
              Transfer any amount to this account and your wallet will be funded automatically
              within seconds.
            </p>
          </div>
        </div>
...
export function FundWalletModal({ open, onOpenChange }: Props) {
  ...
  return (
    <Modal
      ...
      title="Fund Wallet"
      description="Choose your preferred funding method."
    >
      {content}
    </Modal>
  );
